...
def plot_distribution_of_probabilities_edge(
    ...
    bar_container = plt.bar(list(binary_paths),list(binary_prob),color=colors)
    plt.tick_params("x",rotation=90)
    plt.xticks((np.arange(len(binary_paths))),binary_paths)
...
def plot_distribution_comparison(
    ...
    legend = []
    colors = []
    color = iter(cm.rainbow(np.linspace(0, 1, sumDir)))
